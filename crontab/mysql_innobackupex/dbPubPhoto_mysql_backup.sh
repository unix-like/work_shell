...
MYSQL_USER="root"
MYSQL_PWD='uL:2p9RLBH$fi~PH'
MYSQL_SOCKET="/gotwo_data/mysql/dbPubPhoto/socket/mysqld.sock"
MYSQL_PORT=40005
MYSQL_CNF_FILE="/gotwo_data/mysql/dbPubPhoto/etc/my40005.cnf"
INSTNANCE_NAME="dbPubPhoto"

BACKUP_DIR="/gotwo_data/backup/mysql/mysql_innobackupex/dbPubPhoto"
...
BINARY_BACKUP()
{
   ...
   $INNOBACKUPEX --defaults-file=$MYSQL_CNF_FILE  --user=$MYSQL_USER  --password=$MYSQL_PWD --socket=$MYSQL_SOCKET --slave-info  --parallel=$RUN_THREAD_NUM --compress --compress-threads $COMPRESS_THREAD_NUM --no-version-check --stream=xbstream  $BACKUP_DIR  > $BACKUP_DIR/$TRANSFER_FILE
    ...
    then
           ...
           echo "Backup $INSTNANCE_NAME(go2plus_photography,publish,nantong) completed successfully !" |  mailx -s "$INSTNANCE_NAME backup status" $SENTO
       else
