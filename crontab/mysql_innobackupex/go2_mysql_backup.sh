...
MYSQL_SOCKET="/gotwo_data/mysql/dbGo2/socket/mysqld.sock"
MYSQL_PORT=40003
MYSQL_CNF_FILE="/gotwo_data/mysql/dbGo2/etc/my40003.cnf"
INSTNANCE_NAME="dbGo2"

BACKUP_DIR="/gotwo_data/backup/mysql/mysql_innobackupex/db_go2"
...
BINARY_BACKUP()
{
   ...
   $INNOBACKUPEX --defaults-file=$MYSQL_CNF_FILE  --user=$MYSQL_USER  --password=$MYSQL_PWD --socket=$MYSQL_SOCKET --slave-info  --parallel=$RUN_THREAD_NUM --compress --compress-threads $COMPRESS_THREAD_NUM --no-version-check --stream=xbstream  $BACKUP_DIR  > $BACKUP_DIR/$TRANSFER_FILE
    ...
    then
           ...
           echo "Backup $INSTNANCE_NAME(db_go2,db_3e3e) completed successfully !" |  mailx -s "$INSTNANCE_NAME backup status" $SENTO
       else
